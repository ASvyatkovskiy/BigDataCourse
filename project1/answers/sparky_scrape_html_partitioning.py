...
def main(argv,npartitions):
    ...
    fIn_rdd = sc.wholeTextFiles("/user/alexeys/BigDataCourse/web_dataset/2/",npartitions).map(parse_page).map(lambda x: json.dumps(x))
    fIn_rdd.repartition(1).saveAsTextFile('/user/alexeys/BigDataCourse/web_dataset_preprocessed/')

if __name__ == "__main__":
   import time
   start = time.time()
   #Try a few values for the number of partitions in RDD (the default is 2)
   #Try 10, 15 and 50 for  instance
   npartitions = 10
